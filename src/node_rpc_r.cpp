...
void RaftNode::do_pre_election(std::lock_guard<std::mutex> & guard){
    ...
    debug_node("Lost Leader of (%s). Start pre election term: %lu, will timeout in %llu ms. My last_log_index = %lld\n", 
        leader_name.c_str(), tmp_term, default_election_fail_timeout_interval, last_log_index());
    leader_name = "";
    ...
    for (auto & pp : peers) {
        ...
        request.set_term(tmp_term);
        request.set_last_log_index(last_log_index());
        request.set_last_log_term(last_log_term());
        request.set_time(get_current_ms());
        request.set_is_prevote(true);
...
void RaftNode::on_vote_response(const raft_messages::RequestVoteResponse & response) {
    ...
    GUARD
    if (state != NodeState::Candidate && state != NodeState::PreCandidate) {
        return;
    }
    ...
    if(peer.voting_rights){
        ...
        if (response.term() > current_term) {
            ...
            if(test_election()){
                ...
                //prevote
                if(state == NodeState::PreCandidate){
                    reset_peers_vote(guard);
                    do_election(guard);
                }
                else{
                    become_leader(guard);
