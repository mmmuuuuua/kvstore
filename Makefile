...
test_pure: $(GRPCOBJS) $(OBJS)
	$(CXX) $(CFLAGS) $(LOGLEVEL) $(SRC_ROOT)/test/test.cpp -o $(ROOT)/test -pthread $(GRPCOBJS) $(OBJS) /usr/local/lib/libgtest.a  /usr/local/lib/librocksdb.a -I/usr/local/include $(PLATFORM_LDFLAGS)  $(EXEC_LDFLAGS) $(LDFLAGS)

atomic:
	cd $(SRC_ROOT)/test/atomic/ && make atomic DEBUG=normal

$(OBJ_ROOT)/grpc/%.o: $(SRC_ROOT)/grpc/%.cc $(GRPCCODES) $(OBJ_ROOT)/grpc 
	$(CXX) $(CFLAGS) $(LOGLEVEL) -c -o $@ $< /usr/local/lib/librocksdb.a -I/usr/local/include -std=c++17 $(PLATFORM_LDFLAGS)  $(EXEC_LDFLAGS) $(LDFLAGS)

$(OBJ_ROOT)/%.o: $(SRC_ROOT)/%.cpp $(GRPCCODES) $(OBJ_ROOT) 
	$(CXX) $(CFLAGS) $(LOGLEVEL) -c -o $@ $< /usr/local/lib/librocksdb.a -I/usr/local/include -std=c++17 $(PLATFORM_LDFLAGS)  $(EXEC_LDFLAGS) $(LDFLAGS)

$(DYOBJ_ROOT)/grpc/%.o: $(SRC_ROOT)/grpc/%.cc $(GRPCCODES) $(DYOBJ_ROOT)/grpc 
	$(CXX) $(CFLAGS) $(LOGLEVEL) -fPIC -c -o $@ $< /usr/local/lib/librocksdb.a -I/usr/local/include  -std=c++17 $(PLATFORM_LDFLAGS)  $(EXEC_LDFLAGS) $(LDFLAGS)
